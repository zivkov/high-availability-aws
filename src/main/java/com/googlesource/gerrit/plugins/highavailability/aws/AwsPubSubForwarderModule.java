...
public class AwsPubSubForwarderModule extends LifecycleModule {
  ...
  Region getRegion(Configuration config) {
    return Region.of(config.identity().region());
  }
  ...
  AwsCredentialsProvider getCredentialsProvider(Configuration config) throws IOException {
    String accessKey = Files.readString(config.identity().accessKeyIdLocation()).trim();
    String secretKey = Files.readString(config.identity().secretAccessKeyLocation()).trim();
  ...
  ExecutorService getMessageProcessingExecutor(Configuration config) {
    ExecutorService executor =
        Executors.newFixedThreadPool(config.limits().messageProcessingThreads());
    return executor;
  }
